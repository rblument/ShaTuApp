...
public class EncodeAscii extends Objective {
    ...
    public TutorReply example(TutoringSession session, String jsonData) {
        System.out.println("Start tutor newEncodeAsciiexample");

        // parse input safely
        EncodeAsciiStep step = null;
        try {
            if (jsonData != null && !jsonData.isBlank()) {
                step = gson.fromJson(jsonData, EncodeAsciiStep.class);
            }
        } catch (Exception ignored) {
            // if jsonData is malformed, fall back to a fresh step
        }
        if (step == null) {
            step = new EncodeAsciiStep();
        }

        // decide question
        String question = step.getQuestion();
        if (question == null || question.isEmpty()) {
            System.out.println("Question was empty");

            int messageLength = step.getMessageLength();
            if (messageLength <= 0) {
                messageLength = 20; // safe default
            }
    ...
    // compute result
    step.setResult(toBinaryFunction(question));

    System.out.println(step.getResult());

    return genericExample(
            step,
            StepSubType.ENCODE_ASCII,
            ProblemType.ASCII_ENCODE,
            KnowledgeComponentKind.ASCII_ENCODE,
            "Convert the question to binary"
    );
}
    /**
